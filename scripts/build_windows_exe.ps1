...
$WindowsZip = Join-Path $ReleaseDir "SysGuard-7.5-Windows.zip"
$IconArgs = @()
...
Compress-Archive -Path (Join-Path $ProjectRoot "dist\SysGuard\*") -DestinationPath $WindowsZip

Write-Host "Built Windows executable at: $ProjectRoot\dist\SysGuard\SysGuard.exe"
Write-Host "Built Windows release archive at: $WindowsZip"
